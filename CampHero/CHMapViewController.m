...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ( [[CHSearchStore sharedStore] shouldResetMap]) {
        
        if ( [[CHSearchStore sharedStore] locationIsUser]) {
            // Location is current user. Reframe accordingly.
            [self reframeMapViewAroundUser:self.mapView];
            // Hide the search reset button again
            self.resetLocationButton.hidden = YES;
            [[CHSearchStore sharedStore] mapWasReset];
            
        } else if ([[CHSearchStore sharedStore] lastSearchWasTextSearch]) {
...
- (IBAction)searchThisArea:(id)sender
{
    ...
    // Send the search to CHSearchStore using the areaSearch method
    [[CHSearchStore sharedStore] mapAreaSearch:self keywords:keywords distance:distanceString];
...
- (void)createMarkers:(NSMutableArray *)campsites
{
    ...
    for(NSDictionary *campsite in campsites) {
        CHMapMarker *marker = [[CHMapMarker alloc] init];
        ...
        if (![rawPhoneNumber isKindOfClass:[NSNull class]]) {
            marker.subtitle = [[CHSearchStore sharedStore] formatPhoneNumber:rawPhoneNumber];
        }
...
{
    // Get the center
    CLLocation *userInitLocation = [[CHSearchStore sharedStore] userLocation];
...
- (MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id <MKAnnotation>)marker
{
	...
	if ([marker isKindOfClass:[CHMapMarker class]])
	{
...
- (void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control
{
    ...
    CHMapMarker *marker = [view annotation];
    ...
    CHCampsiteViewController *detailVC = [[CHCampsiteViewController alloc] init];
...
- (void)addNotices
{
    ...
    if ([[CHSearchStore sharedStore] noWifiError]) {
        ...
        self.noticeLabel.hidden = NO;
    } else if ([[CHSearchStore sharedStore] noPermissionError]) {
        self.noticeLabel.text = @"Your device won't let me access your current location.";
        self.noticeLabel.hidden = NO;
    } else if ([[CHSearchStore sharedStore] searchFailedError]) {
